  ...
  const barData = data.map((d) => ({
    ...
    topLabelComponent: () => (
      <Text style={{ fontSize: 12, fontWeight: '600', color: '#e5e7eb', marginBottom: 3 }}>
        {d.value >= 60 ? `${Math.floor(d.value / 60)}h${d.value % 60 > 0 ? Math.round(d.value % 60) + 'm' : ''}` : `${d.value}m`}
      </Text>
    ),
  }));
    ...
    <View>
      <View className="flex-row justify-between mb-3">
        <View>
          ...
          <Text className="text-sm font-bold text-gray-200">{avg} min</Text>
        </View>
        <View className="items-end">
          <Text className="text-xs text-gray-400">Peak period</Text>
          <Text className="text-sm font-bold text-violet-400">{peak} min</Text>
        </View>
      </View>
